...
| `transcript.txt` | 最终用于总结的文本。可能来自字幕，也可能来自 Whisper 转写。 |
| `summary.md` | 大模型生成的中文总结。只有调用 LLM 成功时生成。 |
...
| `--language` | `zh` | Whisper 识别语言。中文视频默认不用配置；英文可传 `en`；自动识别传 `auto`。 |
...
中文视频建议保持默认 `small`。快速测试可以用 `tiny`。英文视频可以用 `.en` 后缀模型。
